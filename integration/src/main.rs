...
use sc_service::{LocalCallExecutor, TFullClient, TaskManager};
use sp_core::sr25519;
...
async fn main() {
	...
	let mut builder =
		StandAloneBuilder::<TestBlock, TestRtApi, TestExec, _, _>::new(backend, client.clone());
	...
	let cid = move |parent: H256, ()| {
		...
		async move {
			...
			let timestamp =
				FakeTimestamp::new(0, Duration::from_secs(12).as_millis() as u64, Some(0));
	...
	let num = builder
		.with_state(|| {
			let (d1, d2) = (
				frame_system::Account::<Runtime>::get(AccountId32::from(sender.clone())),
				frame_system::Account::<Runtime>::get(AccountId32::from(receiver.clone())),
			);
	...
	let xt = builder.with_state(|| {
		...
		build_transfer(
			signer,
			AccountId32::from(sender.clone()),
			AccountId32::from(receiver.clone()),
		)
	});
...
> {
	...
	let xt = signer.signed_ext(
		polkadot_runtime::Call::Balances(pallet_balances::Call::transfer {
